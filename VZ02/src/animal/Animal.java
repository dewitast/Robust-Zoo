...
public class Animal {
  ...
  /**
    ...
    * @param jenisMakanan Carnivore, Omnivore, atau Herbivore.
    * @param tempatHidup Hanitat Animal.
    ...
    * @param jumlahHabitat Jumlah habitat yang dapat ditinggali oleh Animal
    * @param jinak Menyatakan jinak atau buasnya Animal.
    * @param cetak Karakter yang merepresentasikan Animal.
    */

  public Animal(Point posisi, String spesies, String suara, String jenisMakanan,
      String[] tempatHidup, double berat, double daging, double sayur,
      int jumlahHabitat, boolean jinak, char cetak) {
    position = posisi;
    species = spesies;
    sound = suara;
    foodType = jenisMakanan;
    numberHabitat = jumlahHabitat;
    habitat = new String[numberHabitat];
    for (int i = 0; i < numberHabitat; i++) {
      habitat[i] = tempatHidup[i];
    }
    weight = berat;
    foodMeat = daging;
    foodVeggie = sayur;
